...
class GridViewController: UIViewController {
    ...
    @IBAction func unwindToGridLoadVC(segue:UIStoryboardSegue) { }
    
    
    func loadQuestions(completion: @escaping (_ success: Bool) -> Void)
    {
        //startTimer()
        var kPathURL:String
        var tPathURL:String
        ...
        case 0  :
            kPathURL = "https://opentdb.com/api.php?amount=50&difficulty=easy&type=multiple"
            tPathURL = "https://opentdb.com/api.php?amount=50&difficulty=easy&type=boolean"
        case 1  :
            kPathURL = "https://opentdb.com/api.php?amount=50&difficulty=medium&type=multiple"
            tPathURL = "https://opentdb.com/api.php?amount=50&difficulty=medium&type=boolean"
        case 2  :
            kPathURL = "https://opentdb.com/api.php?amount=50&difficulty=hard&type=multiple"
            tPathURL = "https://opentdb.com/api.php?amount=50&difficulty=hard&type=boolean"
        default :
            kPathURL = "https://opentdb.com/api.php?amount=50&type=multiple"
            tPathURL = "https://opentdb.com/api.php?amount=50&type=boolean"
        }
        
        // Asynchronous Http call to your api url, using URLSession:
        URLSession.shared.dataTask(with: URL(string: tPathURL)!) { (data, response, error) -> Void in
            ...
            if error == nil && data != nil {
                do {
                    ...
                    self.tieBreakArray = json["results"] as! [NSDictionary]
                ...
                if let path = Bundle.main.path(forResource: "tiebreaker", ofType: "json") {
                    do {
        ...
        // Asynchronous Http call to your api url, using URLSession:
        URLSession.shared.dataTask(with: URL(string: kPathURL)!) { (data, response, error) -> Void in
            // Check if data was received successfully
            if error == nil && data != nil {
                do {
                    // Convert to dictionary where keys are of type String, and values are of any type
                    let json = try JSONSerialization.jsonObject(with: data!, options: .mutableContainers) as! Dictionary<String, AnyObject>
                    // Access specific key with value of type String
                    self.questionsArray = json["results"] as! [NSDictionary]
                    //self.lblQuestions.text = str;
                    completion(true)
                ...
                if let path = Bundle.main.path(forResource: "trivia", ofType: "json") {
                    do {
                        let data = try Data(contentsOf: URL(fileURLWithPath: path))
                        let json = try JSONSerialization.jsonObject(with: data, options: .mutableContainers) as! Dictionary<String, AnyObject>
                        // Access specific key with value of type String
    ...
    @objc func go(){
        progressBar.progress += 0.096; //10 sec

        if (progressBar.progress >= 0.32 && progressBar.progress < 0.64){
            lblProgress.text = "Scouting the enemy..."
        }else if (progressBar.progress >= 0.64 && progressBar.progress < 1.0){
            lblProgress.text = "Preparing for battle..."
            
        }
        else if (progressBar.progress > 0.80 && loadCompleted){
            DispatchQueue.main.async() {
                [unowned self] in
                self.performSegue(withIdentifier: "gameSessionSegue", sender: self)
            }
        }
    }
    
    func progressMessages(){
        lblProgress.text = "Setting up the battlefield..."
        triviaDifficulty = UserDefaults.standard.integer(forKey: "triviaLevel")
        startTimer()
        loadQuestions { (success) -> Void in
            if success {
    ...
    override func prepare (for segue: UIStoryboardSegue, sender: Any?){
        if segue.identifier == "gameSessionSegue"{
            ...
            controller.questionsArray = questionsArray
            controller.tieBreakArray = tieBreakArray
        }
    }
